...
| MT-006-U01 | Usability defect | Medium | **Create selected candidates** appears active when no candidate is selected. | Disable the action until at least one eligible row is selected and announce the selected count in its label or nearby summary. | Implemented — browser retest pending |
| MT-006-U02 | Improvement | Medium | There is no quick way to select all clean eligible rows in a larger batch. | Add **Select all ready** while keeping exception and duplicate rows unselected and individually reviewable. | Implemented — browser retest pending |
