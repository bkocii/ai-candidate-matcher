...
| --- | --- | --- | --- | --- | --- |
| MT-001-F01 | Improvement | Medium | The page has no visible recovery route for a user who cannot sign in. | Before external production use, add a controlled password-recovery flow or clearly document administrator-assisted recovery. Do not introduce public signup. | Resolved — browser tested 2026-09-06 |
| MT-001-V01 | Improvement | Low | The password field has no show/hide control. | Consider adding an accessible password-visibility toggle after higher-priority workflow testing. | Implemented — browser retest pending |
...
recruiters. A task-first panel provides **Add candidates** and **Create vacancy**
immediately. The client metric exposes **Manage client companies** to
administrators, the generic workspace panel is removed, and optional team/client/
settings actions use short non-wrapping labels. Administrator copy explicitly
states that the same account can perform recruiter work.
...
| ID | Type | Priority | Finding | Recommendation | Status |
| --- | --- | --- | --- | --- | --- |
| MT-005-V01 | Visual defect | High | The native browser file picker looks unfinished and does not provide a professional upload surface or selected-file summary. | Use an accessible drag-and-drop upload area with a normal browse fallback, accepted-format/size guidance, and a removable selected-file list. | Implemented — browser retest pending |
| MT-005-U01 | Usability defect | Medium | **Validate and add CVs** appears active when no file is selected, and “validate” exposes an internal safety step rather than the recruiter's outcome. | Disable submission until files are selected and label the action **Add CVs**; report validation transparently after submission. | Implemented — browser retest pending |
| MT-005-U02 | Improvement | Medium | Optional CSV mapping occupies an equally prominent full card even when the recruiter is using the standard CV-only workflow. | Keep initial CV upload primary. Once review rows exist, place mapping after review under **Advanced: Match details from CSV**, explain that it augments uploaded CVs rather than replacing direct CSV candidate import, and keep it collapsed until requested. | Browser tested — passed 2026-09-07 |
| MT-005-V02 | Visual defect | Medium | Large summary cards consume substantial vertical space before the upload and review tasks, including when only one item exists. | Use a compact status row for pending, created, and discarded counts throughout intake. | Browser tested — passed 2026-09-07 |
...
| MT-005-C01 | Improvement | Low | **Apply exact CSV mappings** is accurate but technical wording. | Prefer **Import candidate details from CSV**, retaining exact-filename behavior in the helper text. | Implemented — browser retest pending |
...
| ID | Type | Priority | Finding | Recommendation | Status |
| --- | --- | --- | --- | --- | --- |
| MT-006-V01 | Visual defect | High | The candidate review table requires horizontal scrolling on a 1920px desktop. Selection and filenames disappear when source-reference/discard controls are visible, so a recruiter cannot review one row as a whole. | Replace the wide editable table with responsive candidate cards or a compact grid that keeps status, editable identity, selection, and row actions visible together. | Browser tested — passed 2026-09-07 |
| MT-006-V02 | Visual defect | Medium | Long filenames wrap across several lines in a narrow column and dominate each row. | Show a shortened filename with the complete value available through a tooltip or expandable detail. | Browser tested — passed 2026-09-07 |
| MT-006-C01 | Usability defect | Medium | **Routine** and **CV / exceptions** are internal-sounding labels that do not clearly tell the recruiter whether a row can be created. | Use direct states such as **Ready**, **Needs review**, **Possible duplicate**, or **Cannot create**, with a concise reason beside exceptions. | Browser tested — passed 2026-09-07 |
| MT-006-U01 | Usability defect | Medium | **Create selected candidates** appears active when no candidate is selected. | Disable the action until at least one eligible row is selected and announce the selected count in its label or nearby summary. | Browser tested — passed 2026-09-07 |
| MT-006-U02 | Improvement | Medium | There is no quick way to select all clean eligible rows in a larger batch. | Add **Select all ready** while keeping exception and duplicate rows unselected and individually reviewable. | Browser tested — passed 2026-09-07 |
| MT-006-V03 | Visual defect | Medium | Upload success, file rejection, and private-storage confirmation use nearly identical neutral banners. | Apply accessible success/warning/error treatments with icons or clear status labels while retaining sufficient contrast. | Implemented — browser retest pending |
| MT-006-C02 | Usability defect | Low | The rejection message identifies only **File 1**, forcing the recruiter to infer which selected file failed. | Display the rejected filename in the immediate response without retaining its private bytes. | Implemented — browser retest pending |
| MT-006-C03 | Improvement | Low | **Queue only newly created CVs for background AI profile drafts** is long and implementation-oriented. | Use **Create AI profile drafts in background** and keep the targeting/safety explanation as helper text. | Browser tested — passed 2026-09-07 |
| MT-006-U03 | Workflow hierarchy | Medium | After CVs are supplied during batch creation, the detail page repeats a dominant **Add CVs** section before showing the candidates that now require review. | Make candidate review the primary next task. Move repeat upload below it as collapsed **Add more CVs**, while retaining a full upload surface only for an empty open batch. | Browser tested — passed 2026-09-07 |
...
- **Automated pass:** successful mapping uses Post/Redirect/Get, and the bounded one-time report is consumed on the first detail-page GET; refresh cannot repeat the POST operation.
...
- **Viewport:** Desktop, 1920 × 1080
- **Visual status:** Pass after improvements
...
| ID | Type | Priority | Finding | Recommendation | Status |
| --- | --- | --- | --- | --- | --- |
| MT-009-U01 | Usability defect | Medium | The job is queued, but the summary cards omit a queued or running count; all three outcome cards show zero. | Include **Queued** and **Running** in the compact status summary so the current total is immediately explained. | Browser tested — passed 2026-09-07 |
| MT-009-U02 | Usability defect | Medium | The status page has no visible refresh action, last-updated indicator, or automatic update behavior. A recruiter can continue seeing **Waiting for worker** after processing completes. | Add lightweight polling or a clear **Refresh status** action with a last-updated timestamp, stopping automatically at a terminal state. | Browser tested — passed 2026-09-07 |
| MT-009-U03 | Improvement | Low | The page does not provide a direct route back to the intake batch that created the job. | Add a contextual **Back to intake** link when a job originated from an intake batch. | Browser tested — passed 2026-09-07 |
| MT-009-V01 | Improvement | Low | The large green safety banner has greater visual emphasis than the actual queued item status. | Reduce it to a compact informational note after the safety model is familiar elsewhere in the product. | Browser tested — passed 2026-09-07 |
...
MT-016-U06 is implemented. On a draft vacancy, **Confirm and open vacancy** is
the routine primary action and performs both lifecycle changes in one
transaction; **Confirm only** remains available. Both paths return to a focused
success summary near the top of vacancy detail with the appropriate **Evaluate
candidates** or **Open vacancy** next action. A failed open transition rolls back
the confirmation. Browser acceptance is pending.
...
| MT-021-U06 | Usability defect | High | After recording a decision, the page returns to the top. The saved decision, history, and newly enabled outreach action are several screens below, so the recruiter receives no immediate in-context confirmation or next step. | Redirect to and focus a **Decision saved** summary containing the current decision and next valid action. After approval, place **Generate outreach draft** beside that summary; after reject/revisit, offer **Next candidate** or **Back to review queue**. | Resolved — user confirmed 2026-09-20 |
...
Implementation note (2026-09-17): the five Open MT-021 findings were delivered
as one decision-review batch. The assessment summary, review focus, compact
Matches/Gaps/Verify accordions, and sticky decision surface now share one main
workspace. Decision controls are selectable cards with notes directly below.
After saving, a focused **Decision saved** summary shows the immutable current
version and the next valid action. The empty new-decision form is replaced by a
collapsed **Change decision** action, while decision/assessment history and
eligible outreach remain secondary disclosures. The compatible U02/U03/C01
proposals were absorbed into the same presentation pass. Decision versioning,
scoring, evidence, outreach generation, and tenant boundaries are unchanged.
...
Implementation note (2026-09-20): the routine outreach workflow now has two
human-controlled stages. Recording an approval prepares an email only when the
candidate has an email address and current source records permit contact. The
composer combines the exact recipient, editable subject/body, compact contact
status, and final action; audit and version history are collapsed. **Open in
email app**, copy, and export each recheck currentness and contact permission and
record approval of the exact draft version. The email-app handoff records its
...
covered by the audited action and regression tests. Browser confirmed
2026-09-21.
...
| ID | Type | Priority | Finding | Recommendation | Status |
| --- | --- | --- | --- | --- | --- |
| MT-023-U01 | Usability defect | High | The queue defaults to **Decision pending (0)** and displays an empty state even though **Needs focus (1)** contains the only actionable assessment. The recruiter must notice and click another scope before seeing work. | Default to a unified **Needs attention** queue combining pending decisions, evidence gaps/uncertainties, and changed inputs. If kept separate, automatically open the first non-empty priority scope. | Resolved — user confirmed 2026-09-21 |
| MT-023-U02 | Usability defect | Medium | A green traffic-light badge appears beside an item explicitly classified as **Needs focus** with one gap and five uncertainties. Green can be read as “no review needed” even though it represents only the AI signal. | Label it **AI assessment: Green · 75** and keep **Needs attention** as the dominant workflow status. Never use the AI traffic light as the card's overall state. | Resolved — user confirmed 2026-09-21 |
| MT-023-V01 | Visual defect | Medium | Five large summary cards do not fit one row; **Revisit later** wraps alone onto a second row and leaves substantial empty space. | Use one compact status strip or smaller inline metrics, prioritizing **Needs attention**, **Pending**, **Approved**, and total. Move less-used counts into filters. | Resolved — user confirmed 2026-09-21 |
| MT-023-C01 | Usability defect | Low | The decision badge reads **Decision: Approve**, using the action label rather than the saved state. | Display **Decision: Approved**; use **Approve** only on an action control. | Resolved — user confirmed 2026-09-21 |
| MT-023-C02 | Usability defect | Medium | The bottom notice says outreach remains a separate, later approved action. That wording reinforces the workflow now approved for simplification and conflicts with **Approve candidate and prepare email → Review and send**. | Update the notice after the workflow redesign: decisions remain human-controlled, and preparing an email does not send it; the reviewed send/handoff action is the explicit final boundary. | Resolved — user confirmed 2026-09-21 |
| MT-023-V02 | Improvement | Low | The full recruiter-review recommendation occupies a large highlighted block inside every queue card. This will make multi-candidate queues vertically long. | Show a one- or two-line focus summary with **Show details**; keep gap/verify counts and the primary review action visible. | Resolved — user confirmed 2026-09-21 |

Implementation note (2026-09-20): **Needs attention** is now the default queue
and combines pending decisions, changed inputs, and evidence exceptions. A
compact four-value strip prioritizes attention, pending, approved, and total;
rejected/revisit counts remain secondary. Each card makes **Needs attention**
the workflow state and labels the separate AI signal explicitly. Saved decision
badges use **Approved**, **Rejected**, or **Revisit later**. The full review
recommendation is collapsed under **Show review details**, and the final notice
matches the two-stage email workflow without implying delivery.
...
| ID | Type | Priority | Finding | Recommendation | Status |
| --- | --- | --- | --- | --- | --- |
| MT-035-S01 | Access-control usability defect | High | A single username silently selects between linking an existing account and creating a new account. A mistaken match can grant the wrong person access to candidate data, while an unmatched typo unexpectedly starts account creation. | Make the choice explicit: primary **Invite new recruiter** by email, with a secondary **Add existing account** option. For an existing account, search and show verified identity details before **Grant recruiter access**. | Resolved — browser tested 2026-09-03 |
| MT-035-S02 | Security defect | High | A new recruiter receives a manually shared temporary password, but the workflow does not force a password change before workspace access. The credential may remain valid indefinitely. | Replace shared passwords with an expiring one-time password-setup invitation. Until email delivery exists, enforce a server-side **must change password** state and support a one-time setup link handled outside ordinary candidate pages. | Interim safeguard implemented — browser retest pending |
| MT-035-U01 | Usability defect | High | Confirmed: browser autofill populated username and temporary password during existing-user linking. The first submission failed with **Leave password fields blank when adding an existing account**, alongside the generic temporary-password help text. | Split the two workflows and apply correct autocomplete semantics. Existing-account lookup must not render password inputs at all; after a verified account is selected, one **Grant recruiter access** action should succeed on the first submission. The invitation flow should not ask an administrator to invent a password. | Resolved — browser tested 2026-09-03 |
| MT-035-U02 | Workflow defect | Medium | Existing-user linking displays six fields even though only an account selection is needed. A new-user flow also asks for username and two password fields that email invitation/password setup can eliminate. | Use progressive disclosure in one page: **Invite new recruiter** requires email and optional name; **Add existing account** requires account search and identity confirmation. Generate internal usernames where necessary and keep the final action to one confirmation. | Interim flow implemented — browser retest pending |
| MT-035-V01 | Visual/usability defect | Medium | The long single-column form extends beyond one desktop viewport, placing the primary action at the bottom after several fields that may be irrelevant. | After separating the workflows, show only relevant inputs in a compact card. Keep the organization name and access level visible near the action so the administrator can confirm who receives access and where. | Resolved after switch-alignment correction — browser tested 2026-09-03 |
| MT-035-C01 | Product consistency | Medium | Recruiter creation and administrator creation use the same risky combined account workflow but are documented as separate screens. Fixing only one would leave inconsistent onboarding and duplicate logic. | Build one reusable **account access invitation** component with role-specific copy and permissions, then use it for first administrator, additional administrator, and recruiter onboarding. | Partially implemented — shared forms and interaction pattern; invitations pending |
...
| MT-040-U03 | Account recovery defect | High | The sign-in page has no self-service recovery path, so a forgotten password requires operator intervention. | Add Django's generic, one-time email reset flow without account enumeration. A successful reset also satisfies the first-login password requirement. | Resolved in development — production SMTP acceptance pending |
| MT-040-V01 | Visual defect | Medium | On the successful reset page, the **Sign in** action overlaps the confirmation sentence. | Give the completion action its own block and spacing so the message and next action remain visually distinct at desktop and mobile widths. | Implemented — browser retest pending |
