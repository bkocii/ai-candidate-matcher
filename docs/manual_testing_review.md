...
| MT-040-U03 | Account recovery defect | High | The sign-in page has no self-service recovery path, so a forgotten password requires operator intervention. | Add Django's generic, one-time email reset flow without account enumeration. A successful reset also satisfies the first-login password requirement. | Implemented — browser/email-provider retest pending |
